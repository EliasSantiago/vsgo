...
export class AICustomizationManagementEditor extends EditorPane {
	...
	private createContent(): void {
		...
		if (hasSections.has(AICustomizationManagementSection.AIProviders)) {
			...
			const providersBackBar = DOM.append(this.aiProvidersContentContainer, $('.section-back-bar'));
			providersBackBar.appendChild(this.createBackArrowButton());
			this.aiProvidersWidget = this.editorDisposables.add(this.instantiationService.createInstance(AIProvidersWidget));
		...
		if (this.aiProvidersWidget) {
			this.editorDisposables.add(this.aiProvidersWidget.onDidChangeItemCount(count => {
				this.updateSectionCount(AICustomizationManagementSection.AIProviders, count);
			}));
			this.aiProvidersWidget.fireItemCount();
		}
