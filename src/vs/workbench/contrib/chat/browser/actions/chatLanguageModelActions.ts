...
class ConfigureAIProviderAction extends Action2 {
	static readonly ID = 'workbench.action.chat.configureAIProvider';

	constructor() {
		super({
			id: ConfigureAIProviderAction.ID,
			title: localize2('configureAIProvider', 'Configure AI Provider...'),
			category: CHAT_CATEGORY,
			f1: true,
			menu: [{
				id: MenuId.AccountsContext,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (vendors.length === 0) {
			await quickInputService.pick(
				[{ label: localize('noProvidersAvailable', 'No AI providers are currently registered.') }],
				{ placeHolder: localize('noProvidersHint', 'Make sure the chat extension is enabled.') }
			);
			return;
		}

		type VendorPick = IQuickPickItem & { vendor: string };
		const items: QuickPickInput<VendorPick>[] = [];
...
class ManageAIProvidersAction extends Action2 {
	static readonly ID = 'workbench.action.chat.manageAIProviders';

	constructor() {
		super({
			id: ManageAIProvidersAction.ID,
			title: localize2('manageAIProviders', 'Manage AI Providers...'),
			category: CHAT_CATEGORY,
			f1: true,
			menu: [{
				id: MenuId.AccountsContext,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		while (true) {
			const vendors = languageModelsService.getVendors();
			const groups = configurationService.getLanguageModelsProviderGroups();
			const items: QuickPickInput<ProviderItem>[] = [];
			...
			if (groups.length === 0) {
				items.push({
					type: 'separator',
					label: localize('noProvidersConfigured', 'No AI providers configured yet'),
				});
			} else {
				items.push({
					type: 'separator',
					label: localize('configuredProviders', 'Configured providers'),
				});
				const byVendor = new Map<string, ILanguageModelsProviderGroup[]>();
				...
				for (const [vendorId, vendorGroups] of byVendor) {
					const vendor = vendors.find(v => v.vendor === vendorId);
					const vendorLabel = vendor?.displayName ?? vendorId;
					items.push({
						type: 'separator',
						label: vendorLabel,
					});
					for (const group of vendorGroups) {
						const modelCount = languageModelsService.getLanguageModelGroups(vendorId)
							.find(g => g.group?.name === group.name)?.modelIdentifiers.length ?? 0;
						items.push({
							label: group.name,
							description: modelCount === 1
							...
							vendor: vendorId,
							buttons: [
								{ iconClass: ThemeIcon.asClassName(Codicon.gear), tooltip: localize('editProvider', 'Edit provider settings') },
								{ iconClass: ThemeIcon.asClassName(Codicon.trash), tooltip: localize('removeProvider', 'Remove provider') },
							],
						});
					}
				}
			...
			if (result.item.id === ADD_NEW_ID) {
				if (vendors.length === 0) {
					await dialogService.prompt({
						type: 'info',
						message: localize('noVendorsAvailable', 'No AI providers are currently registered.'),
						detail: localize('noVendorsHint', 'Make sure the chat extension is enabled.'),
					});
					continue;
				}
				type VendorPick = IQuickPickItem & { vendor: string };
				const vendorItems: QuickPickInput<VendorPick>[] = vendors.map(v => ({
					label: v.displayName,
					description: v.vendor,
					vendor: v.vendor,
					iconClass: ThemeIcon.asClassName(Codicon.sparkle),
				}));
				const picked = await quickInputService.pick(vendorItems, {
					placeHolder: localize('configureAIProvider.placeholder', 'Select a provider to configure (API key, endpoint, etc.)'),
					matchOnDescription: true,
				});
				if (picked && 'vendor' in picked) {
			...
			if (result.kind === 'remove' && result.item.vendor && result.item.group) {
				const confirmed = await dialogService.confirm({
					type: 'warning',
					message: localize('removeProviderConfirm', "Remove provider \"{0}\"?", result.item.group.name),
					detail: localize('removeProviderConfirmDetail', 'This deletes the stored API key and configuration.'),
...
export function registerLanguageModelActions() {
	...
	registerAction2(ConfigureAIProviderAction);
	registerAction2(ManageAIProvidersAction);
}

export const CONFIGURE_AI_PROVIDER_ACTION_ID = ConfigureAIProviderAction.ID;
export const MANAGE_AI_PROVIDERS_ACTION_ID = ManageAIProvidersAction.ID;
