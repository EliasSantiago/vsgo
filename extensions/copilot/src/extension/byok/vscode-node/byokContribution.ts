...
export class BYOKContrib extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._registerProviders(this._instantiationService).catch(err => {
			this._logService.error(`BYOK: _registerProviders threw: ${err?.stack ?? err}`);
		});
	}

	private async _registerProviders(instantiationService: IInstantiationService) {
		if (this._byokProvidersRegistered) {
			return;
		}
		this._byokProvidersRegistered = true;

		// Known models list is best-effort: fetch failures (offline, blocked CDN, TLS interception) must not
		// prevent BYOK providers from being usable. Providers fall back to discovering models at request time.
		let knownModels: Record<string, BYOKKnownModels> = {};
		try {
			knownModels = await this.fetchKnownModelList(this._fetcherService);
		} catch (err) {
			this._logService.warn(`BYOK: failed to fetch known models list, continuing with empty list. Error: ${err}`);
		}
