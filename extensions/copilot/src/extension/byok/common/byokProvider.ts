...
export function isBYOKEnabled(_copilotToken: Omit<CopilotToken, 'token'>, _capiClientService: ICAPIClientService): boolean {
	// Fork: BYOK is always enabled regardless of Copilot token state so that
	// users can configure providers (Anthropic, OpenAI, Gemini, Ollama, ...)
	// without first signing in to GitHub Copilot.
	return true;
}
