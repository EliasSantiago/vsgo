...
export class DefaultIntentRequestHandler {
	...
	/**
	 * Gets the user's Copilot plan, returning `undefined` if the Copilot token is
	 * unavailable (BYOK-only setups). This prevents the error formatter from
	 * masking the real underlying error (e.g. provider 4xx) with `GitHubLoginFailed`.
	 */
	private async _safeCopilotPlan(): Promise<string> {
		try {
	...
	private async processResult(fetchResult: ChatResponse, responseMessage: string, chatResult: ChatResult | void, metadataFragment: Partial<IResultMetadata>, baseModelTelemetry: ConversationalBaseTelemetryData, rounds: IToolCallRound[]): Promise<ChatResult> {
		switch (fetchResult.type) {
			...
			case ChatFetchResponseType.Canceled: {
				...
				const errorDetails = getErrorDetailsFromChatFetchError(fetchResult, await this._safeCopilotPlan(), outageStatus);
