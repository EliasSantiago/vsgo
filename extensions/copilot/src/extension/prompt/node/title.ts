...
export class ChatTitleProvider implements vscode.ChatTitleProvider {
	...
	async provideChatTitle(
		...
		// Title generation always uses the Copilot fast endpoint. In pure-BYOK setups
		// the Copilot token isn't available, so fail soft instead of surfacing the auth
		// error as a chat result.
		let endpoint;
		try {
			endpoint = await this.endpointProvider.getChatEndpoint('copilot-fast');
		} catch (err) {
			this.logService.warn(`ChatTitleProvider: skipping title because Copilot endpoint is unavailable (${err?.message ?? err})`);
			return '';
		}
