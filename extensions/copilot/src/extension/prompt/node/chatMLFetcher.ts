	...
	public async fetchMany(opts: IFetchMLOptions, token: CancellationToken): Promise<ChatResponses> {
		...
		try {
			...
			if (!payloadValidationResult.isValid) {
				...
				// Copilot token is only required for Copilot-CAPI requests. BYOK endpoints
				// (Anthropic/OpenAI/Gemini/Ollama/...) set their own Authorization header via
				// getExtraHeaders(), so missing Copilot login must not fail the request.
				let copilotToken: Awaited<ReturnType<typeof this._authenticationService.getCopilotToken>>;
				try {
					copilotToken = await this._authenticationService.getCopilotToken();
				} catch {
					// Placeholder token so the missing-key guard further down does not abort the
					// request. BYOK endpoints overwrite the Authorization header in getExtraHeaders().
					copilotToken = { token: 'byok-no-copilot', username: '', isVscodeTeamMember: false } as unknown as Awaited<ReturnType<typeof this._authenticationService.getCopilotToken>>;
				}
