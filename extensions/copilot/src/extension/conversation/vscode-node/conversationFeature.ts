...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		// Always activate so participants register even without a Copilot token.
		// BYOK providers (Anthropic/OpenAI/Gemini/Ollama/...) handle requests
		// when a non-Copilot model is selected in the picker.
		this.logService.info(`ConversationFeature: Activating without Copilot token (BYOK mode supported)`);
		this.activated = true;
		...
		this._disposables.add(authenticationService.onDidAuthenticationChange(async () => {
			...
			// Stay activated regardless: BYOK keeps working independently.
		}));
	}
