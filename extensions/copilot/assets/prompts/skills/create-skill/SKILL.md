...
Save under one of the neutral skill folders (never `.vscode/...`):

- Workspace-scoped: `.agents/skills/<skill-name>/SKILL.md`
- User-scoped (available in every workspace): `~/.agents/skills/<skill-name>/SKILL.md`

Ask which scope the user wants if it is not obvious from context. Folders named after specific assistants (`.copilot/`, `.github/`, `.claude/`) MUST NOT be created — they exist only for compatibility with files the user already had.
